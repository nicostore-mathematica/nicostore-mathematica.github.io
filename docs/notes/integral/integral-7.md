...
解法令  $X = x - x_0$ ， $Y = y - y_0$ ，$X_0, Y_0$  满足 $\begin{cases} 
a_1X_0 + b_1Y_0 + C_1 = 0 \\
a_2X_0 + b_2Y_0 + C_2 = 0 
\end{cases}$ 
...
>
> > $$
> > y' + P(x)y \Rightarrow \frac{dy}{y} = -P(x)dx  ，  \ln|y| = -\int P(x)dx \\ 
...
> > $$
> > C'e^{-\int P(x)dx} - C(x)e^{-\int P(x)dx}P(x) = Q(x)e^{-\int P(x)dx} \\ 
...
>/proof/
>$$
>y' - \frac{\alpha}{x+1} y = e^x (x+1)^{\alpha},\quad P(x) = -\frac{\alpha}{x+1},\quad Q(x) = e^x (x+1)^{\alpha}\\
>\int P(x) dx = -\alpha \ln |x+1|\\
>y = e^{\alpha \ln |x+1|} \left[ \int e^x (x+1)^{\alpha} e^{-\alpha \ln |x+1|} dx + C \right] = (x+1)^{\alpha} (e^x + C)\quad
>$$
>(C 为任意常数)
...
> \int_{0}^{x} f(v) \, dv = \frac{1}{2} x f(x) + x \quad \Rightarrow \quad f(x) = \frac{1}{2} f(x) + \frac{1}{2} f(x) \cdot x + 1\\
> \Rightarrow \quad x f'(x) - f(x) + 2 = 0 \quad \Rightarrow \quad f(x) - \frac{1}{x} f(x) = -\frac{2}{x}\\
...
>
> $$
> (y^*)' = ae^{-x} - (ax + b)e^{-x} = e^{-x}(-ax + a - b)\\
> (y^*)'' = e^{-x}(ax - 2a + b)\\
> $$
> 代入方程
> $$
> e^{-x}[(ax - 2a + b) - 3(-ax + a - b) + 2(ax + b)] = 3xe^{-x} \\ 
> a = \frac{1}{2}, \quad b = \frac{5}{12}, \quad  \quad y = \left(\frac{1}{2}x + \frac{5}{12}\right)e^{-x} + C_1e^x + C_2e^{2x}
> $$
